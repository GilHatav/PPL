...
import { parse as p, isToken, isSexpString, isSexp } from "../shared/parser";
...
export interface LetValueBinding {tag: "LetValueBinding"; var: VarDecl[]; val: CExp;}
export const makeLetValueBinding = (v: VarDecl[], val: CExp): LetValueBinding =>
    ({tag: "LetValueBinding", var: v, val: val});
export const isLetValueBinding= (x: any): x is Binding => x.tag === "LetValueBinding";
...
export interface LetValuesExp {tag: "LetValuesExp"; bindings: LetValueBinding[]; body:CExp[]}
export const makeLetValuesExp = (bindings: LetValueBinding[], body: CExp[]): LetValuesExp =>
...
export const parseL5SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    ...
    makeFailure("Never"); // here continue
...
const parseLetValues = (binding: Sexp, body: Sexp[] ): Result<LetValuesExp> =>{
    if(isArray(binding)){
        const bndgs = parseLetValuesBindings(binding);
        const parsedBody = mapResult(parseL5CExp, body);
        return safe2((a: LetValueBinding[], b: CExp[]) => makeOk(makeLetValuesExp(a,b)))
            (bndgs, parsedBody);
    }  

    else{return makeFailure("binding must be array");}
}
  
const parseSingle = (single: Sexp): Result<LetValueBinding>=>
    ! isArray(single) ? makeFailure("binding must be an Array"):
    //single.length !== 2 ? makeFailure("single has exactly 2 parameters"):
    ! isArray(single[0]) ? makeFailure("var decls must be an Array"):
  safe2((decls: VarDecl[], body: CExp) => makeOk(makeLetValueBinding(decls, body)))
    (mapResult(parseVarDecl, single[0]), parseL5CExp(single[1]));

const parseLetValuesBindings = (binding: Sexp): Result<LetValueBinding[]> =>  
    isEmpty(binding) ? makeFailure('empty binding is not legal'):
    ! isArray(binding) ? makeFailure("binding must be an Array"):
    mapResult(parseSingle , binding);
...
const unparseVarDecls = (vd: VarDecl[]): Result<string> =>   
     bind(mapResult(unparseVarDecl , vd) , (bdgs: string[]) => makeOk(join(" ", bdgs)));
...
const unparseSetExp = (se: SetExp): Result<string> =>
    ...
    const unparseBindings2 = (bindings: LetValueBinding[]): Result<string> =>
    bind(mapResult(bdg => safe2((vd: string, val: string) => makeOk(`((${vd}) ${val})`))(unparseVarDecls(bdg.var), unparse(bdg.val)), bindings),
         (bdgs: string[]) => makeOk(join(" ", bdgs)));


const unparseLetValueExp = (le: LetValuesExp): Result<string> =>
    safe2((bdgs: string, body: string) => makeOk(`(let-values (${bdgs}) ${body})`))
    (unparseBindings2(le.bindings), unparseLExps(le.body));

   // let x = parseL5('(L5 (define f(lambda (x)(values 1 2 3))) (let-values (((a b c) (f 0)))(+ a b c)))') ;
  //  let y = parseL5('(L5 (define f (lambda (x) (values 1 2 3))))') ;
  //  let w = parseL5('(L5 (define f (let-values (((a b c) (f 0))) (+ a b c)))') ;
   // let z = parseL5('(L5 (let-values (((a b c) (f 0))) (+ a b c)))') ;
  //  let f = parseL5('(L5 (let-values (((x y) (values 10 3)) ((w z) (values 7 8))) (list x y w z)))') ;
        // console.log(bind(x , unparse)) ; //Parenthesis Problem!!  
