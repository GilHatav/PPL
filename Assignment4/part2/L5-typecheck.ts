// L5-typecheck
// ========================================================
import { equals, map, zipWith, is } from 'ramda';
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         isPrimOp, isProcExp, isProgram, isStrExp, isVarRef, parseL5Exp, unparse,
         AppExp, BoolExp, DefineExp, Exp, IfExp, LetrecExp, LetExp, NumExp,
         Parsed, PrimOp, ProcExp, Program, StrExp, LetValuesExp, isLetValuesExp } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isTupleTExp, isNonTupleTExpArray, makeNonEmptyTupleTExp, isNonEmptyTupleTExp, isTupleTExpArray, isNonTupleTExp, isNonETupleTExpArray, makeEmptyTupleTExp } from "./TExp";
import { isEmpty, allT, first, rest } from '../shared/list';
import { Result, makeFailure, bind, makeOk, safe3, safe2, zipWithResult, mapResult, isOk, isFailure } from '../shared/result';
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> =>  
    isPrimOp(app.rator) && app.rator.op === 'values' ? typeofValues(app ,tenv): 
   ...
   /* export const typeofValuessss = (exp: AppExp, tenv: TEnv): Result<TExp> => {
        const argsTEs = mapResult((v) => typeofExp(v, tenv), exp.rands);
       return isOk(argsTEs) && isNonTupleTExpArray(argsTEs.value) && isEmpty(argsTEs.value) ? makeOk(makeEmptyTupleTExp()):
       isOk(argsTEs) && isNonTupleTExpArray(argsTEs.value) ? makeOk(makeNonEmptyTupleTExp(argsTEs.value)): 
                makeFailure("wrong!"); 
    };*/
    export const typeofValues = (exp: AppExp, tenv: TEnv): Result<TExp> => {
        const argsTEs = mapResult((v) => typeofExp(v, tenv), exp.rands);
       return isOk(argsTEs) && isNonTupleTExpArray(argsTEs.value) ? makeOk(makeNonEmptyTupleTExp(argsTEs.value)):
                makeFailure("wrong!");  
    };
...
export const typeofLetValuesExp = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
    ...
    const vars = map((b) => b.var, exp.bindings).reduce((acc, val) => acc.concat(val), []);
    const varsS = map((b) => b.var, vars);
    const vals = map((b) => b.val, exp.bindings);
    const valss = (mapResult((b)=> typeofExp(b  ,tenv) , vals));
    const varTEs = map((b) => b.texp, vars);
  
    
    if( isOk(valss) && isTupleTExpArray(valss.value) === true && isNonETupleTExpArray(valss.value) === true ){
       const x1 =   valss.value
       if(isNonETupleTExpArray(x1))  {
        const x2 = map((b)=> b.TEs , x1).reduce((acc, val) => acc.concat(val), []);
         const constraintss = zipWithResult((varTE, val)=> checkEqualType(varTE, val, exp), x2 ,varTEs)
       return bind(constraintss, _ => typeofExps(exp.body, makeExtendTEnv(varsS, varTEs, tenv)));
       } 
    }                   
    return makeFailure("");  
};

export const typeofLetValuesExps = (exp: LetValuesExp, tenv: TEnv): Result<TExp> => {
