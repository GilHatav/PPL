...
export const isNonTupleTExpArray = (x: any): x is NonTupleTExp[] =>
    isArray(x) && x.every(isNonTupleTExp);

    export const isTupleTExpArray = (x: any): x is TupleTExp[] =>
    isArray(x) && x.every(isTupleTExp);

    export const isNonETupleTExpArray = (x: any): x is NonEmptyTupleTExp[] =>
    isArray(x) && x.every(isNonEmptyTupleTExp);
...
export const parseTExp = (texp: Sexp): Result<TExp> =>
    ...
    isArray(texp) && texp.indexOf('->')!== -1  ? parseCompoundTExp(texp):
    isArray(texp) && texp.indexOf('->')=== -1  ? parseTupleTExps(texp): 
...
const parseTupleTExps = (texps: Sexp[]): Result<TupleTExp> =>  {
    const isEmptyTuple = (texps: Sexp[]): boolean =>
    (texps.length === 1) && (texps[0] === 'Empty');
    const a = parseTupleTExp(texps);
    return isEmptyTuple(texps) ? makeOk(makeEmptyTupleTExp()): 
    isOk(a) && isNonTupleTExpArray(a.value) ? makeOk(makeNonEmptyTupleTExp(a.value)): 
    makeFailure("Failure occurs in parsing tuple");
}  
