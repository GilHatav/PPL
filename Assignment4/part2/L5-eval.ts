...
import { CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, isCExp, isLetValuesExp, LetValuesExp } from './L5-ast';
...
import { isAppExp, isDefineExp, isIfExp, isLetrecExp, isLetExp,
         isProcExp, isSetExp , LetValueBinding } from "./L5-ast";
import { applyEnv, applyEnvBdg, globalEnvAddBinding, makeExtEnv, setFBinding,
         theGlobalEnv, Env, FBinding } from "./L5-env";
import { isClosure, makeClosure, Closure, Value, SExpValue, isSExp } from "./L5-value";
...
/*const evalLetValuesExpss = (exp: LetValuesExp, env: Env): Result<Value> => {
    const varDecls = map((d)=> d.var , exp.bindings).reduce((acc, val) => acc.concat(val), []);
    const stringDecls = map((b: VarDecl) => b.var, varDecls);
    const vals = mapResult((v : CExp) => applicativeEval(v, env), map((b : LetValueBinding) => b.val, exp.bindings));
  //  console.log(vals)
    if(isOk(vals)){
        let x1 = vals.value
        //map((b)=> b.TEs , x1)
       
        if(checkArrayOfArrays(x1) ){       
         let x2 = map((b) => b.concat(rest(x1)), x1).reduce((acc, val) => acc.concat(val), [])
            console.log(x2)
            return bind(makeOk(x2), (vals: Value[]) => evalSequence(exp.body, makeExtEnv(stringDecls, vals, env)));
        }
    }

   // const mergeVals : Result<SExpValue[]> = bind(vals , b.concat.apply(b , vals));
    return bind(vals, (vals: Value[]) => evalSequence(exp.body, makeExtEnv(stringDecls, vals, env)));
      
};  */

const evalLetValuesExp = (exp: LetValuesExp, env: Env): Result<Value> => {
    
     exp.bindings.forEach(element => {
        const vars = map((b: VarDecl) => b.var, element.var); 
        const vals = applicativeEval(element.val , env);
       
       if(isOk(vals) && isArray(vals.value)){
        env = makeExtEnv(vars, vals.value , env);        
       }    
    });

    return evalSequence(exp.body,env);
};
