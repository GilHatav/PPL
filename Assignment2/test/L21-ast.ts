...
import { first, rest, second, isEmpty, allT, cons } from '../imp/list';
...
const parseForExp = (params: Sexp[]) : Result<ForExp> => 
    ...
    isEmpty(rest(rest(params)))? makeFailure ("for missing 2 argumens"):
    isEmpty(rest(rest(rest(params))))? makeFailure ("for missing 1 argument"):
    params.length != 4 ? makeFailure("Expression not of the form (for <VarDecl> <NumExp> <NumExp> <CExp>)"):
    parseGoodFor(params[0],rest(params));


const parseGoodFor = (varloop: Sexp, params:Sexp[]): Result<ForExp> =>
    ! isIdentifier(varloop) ? makeFailure("First arg of for must be a var declration" + varloop ) : 
    bind(mapResult(parseL21CExp,params) , (cexps: CExp[])=>
    
        ! (isNumExp(cexps[0])) ? makeFailure("Second arg of for must be a var declartion"): 
        ! (isNumExp(cexps[1])) ? makeFailure("Third arg of for must be a var declration"):
        cexps[0].val > cexps[1].val ? makeFailure("Range is invaild"):
         makeOk(makeForExp(makeVarDecl(varloop),cexps[0],cexps[1],cexps[2]))
...
//const myForExp = bind(parseSexp('(for i 1 (+ 2 2) (* i i))'),parseL21Exp);
//console.log(JSON.stringify(myForExp,null,2));
