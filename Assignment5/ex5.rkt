...
(define sqrt-lzl 
  (lambda (x init)
   (if (< init 0)
       (cons-lzl (cons init (abs (- (square init) x))) (lambda () empty-lzl))
...
(define find-first
  (lambda (lz-lst p)
     (if (empty-lzl? lz-lst) 'fail    
       (
      if (p(head (take lz-lst 1)) ) (head(take lz-lst 1)) (find-first (tail lz-lst) p )) 
    )
  )
)
...
(define sqrt2
  (lambda (x init epsilon)
      ( head (find-first (sqrt-lzl x init ) (lambda (y) (< (cdr y) epsilon )) )
    )
  )
)
...
(define collect-all-values-2
 (lambda (lists key)
    (if (empty? lists)
      (list)
      (if (equal? (get-value$ (car lists) key (lambda (x) #t) (lambda () #f) ) #f)
