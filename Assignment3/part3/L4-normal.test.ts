...
describe('L4 Normal Eval', () => {
    ...
    */
    ...
    it('Divide by zero test', () => {
        expect(bind(parseL4(`
            (L4 (define divideByZero (lambda (x) (/ 1 0)))
            (define g (lambda (x) 13))
            (g (divideByZero 0)))`), evalNormalProgram)).to.deep.equal(makeOk(13));
    });

    it('side effect check', () => {
        expect(bind(parseL4(`
            (L4 (define show (lambda (x) (display x)))
            (define constant (lambda (x) (+ 3 2)))
            (constant (show 7)))`), evalNormalProgram)).to.deep.equal(makeOk(5));
    });

    it('Invalid parameter type passing ', () => {
        expect(bind(parseL4(`
            (L4 (define escapeLoop (lambda (x) (+3 x)))
            (define const (lambda (x) 3))
          (const (escapeLoop #t)))`), evalNormalProgram)).to.deep.equal(makeOk(3));
    });
